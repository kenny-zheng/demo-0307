...
namespace App.BLL
{
    public class User : ServiceBase, IUser
    {
        ...
        public async Task<ResponseBase<string>> UserLogin(UserLoginRequest model, string clientIP)
        {
            ...
            try
            {
                await using (var context = await base.dbTemplate(Enum.ConnectionMode.Slave))
                {
                    ...
                    string jwtToken = JWT.Encode(payload, Encoding.UTF8.GetBytes(secret), JwsAlgorithm.HS512);
        ...
        public async Task<ResponseBase<UserGetDataResponse>> GetData(UserGetDataArgs Args)
        {
        ...
        public async Task<ResponseBase<List<UserGetUserLogResponse>>> GetUserLog(UserGetUserLogArgs Args)
        {
